...
| `app/src/main/java/com/inigo/xudoku/model/Difficulty.kt` | Enum con 5 niveles (`VERY_EASY`→40 celdas/3 pistas, `EASY`→30/3, `MEDIUM`→25/2, `HARD`→20/1, `HARDEST`→17/0). Los valores de celdas y pistas están calibrados. | Solo si se añade un nuevo nivel de dificultad; no cambiar los existentes |
...
| `app/src/main/java/com/inigo/xudoku/ui/screen/DifficultyScreen.kt` | Selección de dificultad: TopAppBar (← SUDOKU ⚙), logo con glow, 5 `DifficultyCard` 3D con escala cromática de intensidad, barra de progreso global con gradient. Desacoplado en `DifficultyScreenContent` para soportar `@Preview` sin inicializar Koin. | Al añadir dificultades o cambiar la selección |
...
| `app/src/main/java/com/inigo/xudoku/ui/screen/VictoryScreen.kt` | Victoria: TopAppBar, confeti (4 colores brand), badge NUEVA MARCA, puntuación, StatCards con íconos (timer/cancel), card de dificultad con dots, barra XP gradient, botones "Siguiente Nivel" y "Menú Principal". Incluye `@Preview` con datos reales | Al cambiar el layout de victoria |
| `app/src/main/java/com/inigo/xudoku/ui/screen/StatsScreen.kt` | Redirige y delega en `ProfileScreen.kt`. *(Consolidada dentro de Perfil)* | Raramente |
| `app/src/main/java/com/inigo/xudoku/ui/screen/ProfileScreen.kt` | Perfil de usuario completo con estadísticas unificadas: Avatar dinámico estilo Origami (Liga + sub-rango I, II, III, IV), nivel, progreso de XP, chips de filtro por dificultad, logros totales, gráfica de evolución de puntos, desglose de dificultad, marcas de tiempo/racha e historial de partidas recientes. | Al implementar autenticación o actualizar visualización de perfil y métricas |
...
| `app/src/main/java/com/inigo/xudoku/ui/components/DifficultyCard.kt` | Tarjeta de dificultad con efecto 3D press (`offset(y = offsetY)` animado). Progresión cromática de intensidad del dolor (`VERY_EASY` Esmeralda → `EASY` Azul → `MEDIUM` Ámbar → `HARD` Carmesí → `HARDEST` Fuego Sangre). Mapeo visual en `difficultyVisuals()` | Al cambiar colores o layout de las tarjetas de dificultad |
| `app/src/main/java/com/inigo/xudoku/ui/components/XudokuBottomBar.kt` | Bottom nav global con 2 tabs (`Play` y `Profile`). Tab activo: círculo `SecondaryContainer`. Nombres externalizados en `strings.xml`. | Al añadir tabs o cambiar iconos |
